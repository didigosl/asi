...
class JSContentView: UIView {
    ...
    fileprivate lazy var collectionView: UICollectionView = {
        ...
        collection.backgroundColor = parentVC.view.backgroundColor
        return collection
    }()
...
extension JSContentView:UICollectionViewDataSource{
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        child.view.frame = cell.bounds
        cell.contentView.addSubview(child.view)
        return cell
    }
}
// MARK: - 点击title代理
extension JSContentView:JS_TitleViewDelegate{
    func titleView(titleView: JS_TitleView, targetIndex: Int) {
        //禁止代理方法
        isForbidDelegate = true
        //切换CollectionItem
       let index =  IndexPath(item: targetIndex, section: 0)
        collectionView.selectItem(at: index, animated: false, scrollPosition: UICollectionView.ScrollPosition.right)
    }
}
